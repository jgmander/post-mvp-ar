...
typedef BoolResultHandler = void Function(bool result);
...
//  UnsupportedHandler onUnsupported;
  ...
  Future<void> primeLandmarks(List<String> placeIds) async {
    try {
      await _channel.invokeMethod('primeLandmarks', {'placeIds': placeIds});
    } on PlatformException catch (ex) {
      print(ex.message);
    }
  }
  ...
  Future<dynamic> _handleMethodCalls(MethodCall call) async {
    ...
    switch (call.method) {
      ...
      case 'onRooftopAnchorResolved':
        ...
        break;
      case 'onCenterHitBuilding':
        if (onCenterHitBuilding != null) {
          // Handle both raw bool (iOS) and Map (Android) argument formats
          if (call.arguments is bool) {
            onCenterHitBuilding!(call.arguments as bool);
          } else if (call.arguments is Map) {
            final args = call.arguments as Map<dynamic, dynamic>;
            onCenterHitBuilding!(args['isBuilding'] as bool);
          }
        }
        break;
      case 'onCompatibilityError':
        if (onCompatibilityError != null) {
          final args = Map<String, dynamic>.from(call.arguments as Map);
          onCompatibilityError!(args);
        }
        break;
      case 'onPlaneTap':
        if (onPlaneTap != null) {
  ...
  Future<Map<String, dynamic>?> getGeospatialPose() async {
    try {
      final result = await _channel.invokeMethod('getGeospatialPose');
      if (result != null) {
        return Map<String, dynamic>.from(result);
      }
    } catch (e) {
      if (debug ?? true) print('Geospatial warning: $e');
    }
    return null;
  }

  Future<void> addEarthAnchorNode(ArCoreNode node, double latitude, double longitude, double altitude,
      {String? parentNodeName}) {
    final params = _addParentNodeNameToParams(node.toMap(), parentNodeName);
    if (params != null) {
    ...
    _addListeners(node);
    return _channel.invokeMethod('addEarthAnchorNode', params);
  }

  Future<void> resolveAnchorOnRooftopAsync(ArCoreNode node, double latitude, double longitude, double altitude,
      {String? parentNodeName}) {
    final params = _addParentNodeNameToParams(node.toMap(), parentNodeName);
    if (params != null) {
      params['latitude'] = latitude;
      params['longitude'] = longitude;
      params['altitude'] = altitude; // Above roof
    }
    _addListeners(node);
