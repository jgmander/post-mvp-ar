...
- [ ] **App Store Connect (iOS):** Complete store metadata, upload final screenshots, and submit for App Review.
- [ ] **Google Play Console (Android):** Finalize Target Audience declaration, upload live screenshots, and promote from Internal Track to Production review.
- [ ] **Custom Domain & Email:** Install `get-post.co` / `spatial-labs.net` and forward `safety@get-post.co` -> `jgmander@gmail.com`.
